...
まず `useAsyncData` です。
こちらは基本的に前述した「Nuxt.js 内部で作成した API」を非同期で呼ぶためのメソッドになります。

「Nuxt.js 内部で作成した API」を作るためにはまず `<プロジェクトのルート>/server/api` ディレクトリ配下に js/ts ファイルを設置する必要があります。
今回は次の関数を作ってみました。
...
![](/images/nuxt3-use-async-data.png)
...
返り値はこんな形で返却されます
※[公式リンク](https://jsonplaceholder.typicode.com/todos/1)。
...
まだパブリックベータ版になったばかりなこともあり、まだドキュメントも含め情報が少ない状態ではありますが、
既存で Nuxt 2 を使っている人たちへのサポートや、今回紹介した Fetch 周りのアプローチが Next.js/Svelte Kit 辺りとは良い意味で別路線に進んでいること、デフォルトで TS 対応がなされているなど個人的には好印象を受けました。
筆者は今後も Nuxt 3 を良い方向に向かうようにしっかり見守っていきます。
